...
def auth_user_setup(self, user=None):
    user = User.objects.create(**test_user) if not user else User.objects.create(**user)
...
class AuthenticationTestCase(APITestCase):
    ...
    def test_get_an_answer(self):
        ...
        q = create_a_question()
        a = Answer.objects.create(user_id=user.id, question_id=q.question_id, **test_answer)
        unauth_user_setup(self)
        response = self.client.get(reverse('get_put_del_an_answer', kwargs={'question_id': q.question_id, 'answer_id': a.answer_id}))
    ...
    def test_auth_calls(self):
        ...
        q = create_a_question()
        a = Answer.objects.create(user_id=user.id, question_id=q.question_id, **test_answer)
        response = self.client.post(reverse('post_a_question'), data=test_question)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        response = self.client.post(reverse('post_an_answer', kwargs={'question_id': q.question_id}), data=test_answer)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        response = self.client.put(reverse('get_put_del_an_answer', kwargs={'question_id': q.question_id, 'answer_id': a.answer_id}), data=test_answer2)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        response = self.client.delete(reverse('get_put_del_an_answer', kwargs={'question_id': q.question_id, 'answer_id': a.answer_id}))
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        response = self.client.delete(reverse('get_put_del_a_question', kwargs={'question_id': q.question_id}))
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        response = self.client.put(reverse('get_put_del_a_question', kwargs={'question_id': q.question_id}), data=test_question2)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
...
class ResponseContentTestCase(APITestCase):
    def test_post_a_question(self):
        ...
        test_question_cp = dict(test_question)
        del test_question_cp['answers']
        response = self.client.post(reverse('post_a_question'), data=test_question_cp)
        resp_payload = response.json()
        self.assertEqual(len(resp_payload), 7)
        for arg in ('question_id', 'created_timestamp', 'updated_timestamp', 'user_id', 'question_text', 'categories', 'answers'):
    ...
    def test_get_an_answer(self):
        user = auth_user = auth_user_setup(self)
        q = create_a_question()
        a = Answer.objects.create(user_id=user.id, question_id=q.question_id, **test_answer)
        response = self.client.get(reverse('get_put_del_an_answer', kwargs={'question_id': q.question_id, 'answer_id': a.answer_id}))
        resp_payload = response.json()
    ...
    def test_get_question(self):
        ...
        response = self.client.get(reverse('get_all_questions'))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.client.get(reverse('get_put_del_a_question', kwargs={'question_id': q.question_id}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class BadRequestHandlingTestCase(APITestCase):
    def test_update_and_delete_others_answer(self):
        ...
        q = create_a_question()
        a = Answer.objects.create(user_id=other_user.id, question_id=q.question_id, **test_answer)
        auth_user_setup(self, test_user2)
        response = self.client.put(reverse('get_put_del_an_answer', kwargs={'question_id': q.question_id, 'answer_id': a.answer_id}), data=test_answer2)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response = self.client.delete(reverse('get_put_del_an_answer', kwargs={'question_id': q.question_id, 'answer_id': a.answer_id}), data=test_answer2)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
